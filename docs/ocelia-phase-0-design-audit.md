...
2. **Two nav items have no designed screens.** The instructor has **two nav levels**: a top nav in the
   ...
   **placeholder stub pages** ("coming soon"); **drop `Quicklink?`** (a placeholder with no defined target).
...
`Profile/Course`, the 5 sub-tabs, and the `Delete course · Save changes` footer (the frame's `Undo` button is dropped by decision — save-only).
...
| **OQ-5** *(resolved 2026-07-17)* | **Global Analytics** & **Global Chats** top-nav items (+ `Quicklink?`) have **no frames**. | **Keep** `Global Analytics` + `Global Chats` as nav items → **placeholder stub pages** ("coming soon" / empty-state) for now (dead links, not fully-designed features). **`Quicklink?` dropped** (a placeholder with no defined target). | Phase 6 |
...
(engagement / reach / clear-data); the `Quicklink?` placeholder is dropped (see the plan decision log). **Phase 1** (tokens/fonts/icons + the
