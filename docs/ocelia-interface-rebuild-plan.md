...
**Status:** Phases 1–7 shipped; **Phase 8 legacy purge done** (MUI fully removed, `AppV2` is the only app) — OCELIA rebrand, performance targets, and WCAG-AA audit remaining · **Last updated:** 2026-07-20
...
> **Update (2026-07-20):** migrated to a **data router** (`createBrowserRouter` + `RouterProvider`) to unlock
> `useBlocker` for the app-wide unsaved-changes guard. Same nested layouts / role guards / lazy routes — the
> route tree is now exported as `routes` (`createRoutesFromElements`) under a `RootLayout` element. See the
> decision-log row + engineering-log ADR-013.
...
| Chat-logs **in-app table + Excel** | today: async job writes a **CSV** to S3 (presigned URLs); no row data, no `.xlsx` | **Rows-as-JSON DONE (B5, 2026-07-20)** — `GET /instructor/course_messages_rows` (paginated, ownership-scoped) backs the in-app Chat History table. **Still pending:** server-side **`.xlsx`** export (extend the `course_messages` job; CSV unchanged) |
...
| Course **term/section** + Create-course **Term** & **primary/secondary instructor** | mockups show a course term/section and a Create-course Term dropdown + primary-vs-secondary instructor; **no schema** backs any of it | **Deferred (B8, surfaced 2026-07-20 parity pass)** — add `term`/`section` course columns + an instructor-role/`is_primary` flag on `Enrolments`; until then term/section render only when present and Create-course uses a flat instructor multi-select |
| **Hard-delete** instructor / course | mockup "Delete instructor" + course delete map to **demote (`lower_instructor`) / unenroll**, not a true delete | **Deferred (B9, surfaced 2026-07-20 parity pass)** — decide if hard-delete is wanted; if so add `DELETE` routes with cascade cleanup (per B6) + Cognito handling; else keep demote/unenroll and keep the confirm copy honest |

Two mockup items are **not** new gaps: Insights **Clear data** is already **B1**; **regenerate access code** — the endpoint exists but is intentionally **not surfaced** in the Students screen per the mockup, so `useRegenerateAccessCode` is currently orphaned (a UI-placement decision, not a backend gap).
...
**Legacy purge — DONE (2026-07-20):** the old MUI interface is fully removed. Deleted `LegacyApp` + the `VITE_UI_V2` flag (`main.jsx` now renders `AppV2` directly and owns `Amplify.configure`), all legacy `src/pages/{admin,instructor,student}` pages + `pages/Login.jsx`/`Container.jsx`, the 11 legacy top-level components, and `Theme.jsx`. Dropped MUI/emotion, `lucide-react`, `material-react-table`, and `@aws-amplify/ui-react` from `package.json` (+ the Roboto `@import` and legacy Tailwind font aliases). No `src/` file imports MUI/emotion/lucide/material-react-table. Gate green: build ✓, lint 0 errors (18 intentional `react-refresh` warnings), 275 tests pass.

**Rebrand — DONE (2026-07-20):** app title → "OCELIA", favicon → the UBC crest (`public/ubc-logo.svg`), `package.json` name → `ocelia-frontend`, README H1/intro → OCELIA; orphaned legacy `public/*.png` + `src/assets/loginframe.png` removed. (`homepage` already points at the Amplify deploy URL.)

**Performance — bundle split DONE (2026-07-20):** added Vite `manualChunks` isolating the heavy vendors (react-pdf, react-markdown, react-syntax-highlighter, katex, recharts, aws-amplify) into their own lazy, cacheable chunks. **StudentChat 1,191 KB → 20 KB, index 572 → 364 KB, InsightsTab 371 → 5 KB**; largest chunk now **239 KB gzipped — under the ≥500 KB-gzipped target** ([§16](#16-performance--success-metrics)). Still pending: **Lighthouse Perf ≥ 90 / A11y ≥ 95 must be run in a browser** (not runnable here) + a formal ≥30%-vs-baseline confirmation.

**A11y — static gate DONE (2026-07-20):** added `eslint-plugin-jsx-a11y` (recommended) to the lint gate; all findings resolved — renamed the `AppHeader` `role` prop → `userRole` (DOM-attribute clash), documented the `no-autofocus` exception (intentional dialog/step focus), taught the label rule our Radix control components, and disabled `heading-has-content` on the `Card`/`Alert` title primitives (content comes from consumers). **Still pending: full WCAG-AA validation needs manual assistive-tech testing + expert review** — automated linting only catches a subset.

**Exit when (remaining):** Lighthouse perf/a11y run in a browser; manual WCAG-AA / assistive-tech audit; ≥30%-vs-baseline bundle confirmation.
...
- **B5** *(rows-as-JSON done 2026-07-20; `.xlsx` pending)* chat-logs: the in-app Chat History table is backed by **`GET /instructor/course_messages_rows`** (paginated, ownership-scoped; `cdk/lambda/lib/instructorFunction.js` + OpenAPI + `test/instructor-chat-history.test.ts`). **Still pending:** server-side **`.xlsx`** export (extend the `course_messages` job; CSV unchanged). Consumed by Phase 6.
...
- **B8** *(deferred — surfaced by the 2026-07-20 parity pass)* Courses schema for mockup fields: course **term/section** + Create-course **Term** dropdown + **primary/secondary instructor** distinction. Nothing backs these today → term/section render only when present; Create-course uses a flat instructor multi-select. Needs `term`/`section` course columns + an instructor-role/`is_primary` flag on `Enrolments` (+ param/route updates).
- **B9** *(deferred — surfaced by the 2026-07-20 parity pass)* hard-delete instructor / course. The mockup "Delete instructor" + course delete currently **demote (`lower_instructor`) / unenroll**, not a true delete. Decide whether hard-delete is wanted; if so add `DELETE` routes with cascade cleanup (reuse B6's mark-and-sweep) + Cognito user handling; otherwise keep demote/unenroll and keep the confirm copy honest.
...
| 2026-07-20 | **Screen-by-screen Figma visual-parity pass** (fileKey `9Kt3RPFlZalJnPtCuHLObU`) across student/instructor/admin. Shipped **B5 rows-as-JSON** (`GET /instructor/course_messages_rows`) backing the in-app Chat History table (**`.xlsx` half still pending**); unified modal chrome (title + footer over dividers, Cancel = `outline`); instructor names render "Lastname, Firstname". **Logged new deferred backend gaps** surfaced by the pass: **B8** (Courses term/section + Create-course Term + primary/secondary instructor — no schema) and **B9** (hard-delete instructor/course vs current demote/unenroll). **Not gaps:** Insights *Clear data* = existing **B1**; the regenerate-access-code endpoint exists but is intentionally unsurfaced per the mockup (`useRegenerateAccessCode` orphaned). A **user-driven manual screen-by-screen correction pass** follows this. |
| 2026-07-20 | **Phase 8 legacy purge executed** (user-requested, ahead of finishing the per-screen polish). Deleted the entire legacy MUI interface: `LegacyApp` + the `VITE_UI_V2` strangler flag, `App.jsx`/`App.css`, all `src/pages/{admin,instructor,student}` pages + `Login`/`Container`, the 11 legacy top-level components, and `Theme.jsx`. `main.jsx` now renders `AppV2` directly and owns `Amplify.configure`. Uninstalled MUI (`@mui/material`, `@mui/icons-material`, `@mui/x-date-pickers`), `@emotion/{react,styled}`, `lucide-react`, `material-react-table`, `@aws-amplify/ui-react`; dropped the Roboto `@import` + legacy Tailwind font aliases. Kept `pages/dev` (StyleGuide/Gallery), `ErrorBoundary`, and the three contexts. Verified: build ✓, lint 0 errors, 275 tests pass. **The `VITE_UI_V2` flag no longer exists — the OCELIA UI is the only interface.** Rebrand + perf + a11y audit (rest of Phase 8) still outstanding. |
| 2026-07-20 | **Phase 8 rebrand + perf + a11y-gate done.** Rebrand: title/favicon/`package.json` name/README → OCELIA; orphaned legacy assets removed. Perf: Vite `manualChunks` split the heavy vendors (pdf/markdown/syntax-highlighter/katex/recharts/amplify) into lazy cacheable chunks — StudentChat 1,191→20 KB, index 572→364 KB, InsightsTab 371→5 KB, largest chunk 239 KB gz (< 500 KB gz target); `chunkSizeWarningLimit` raised to 800 (documented — those two vendor chunks are large raw but small gzipped + lazy). A11y: added `eslint-plugin-jsx-a11y` to the gate, resolved all findings (AppHeader `role`→`userRole`, documented `no-autofocus` exception, `controlComponents` for the label rule, `heading-has-content` disabled on Card/Alert primitives). Gate green: build ✓, lint 0 errors, 275 tests ✓. **Not doable in this environment (still open): Lighthouse Perf≥90/A11y≥95 + manual WCAG-AA/assistive-tech audit + ≥30%-vs-baseline confirmation.** |
| 2026-07-20 | **Admin detail panes → staged "Save changes" (Option A), user-requested — reverses the earlier save-only footer.** `InstructorDetail` + `CourseDetail` now buffer the OCELIA-access toggles, instructor/course add + remove, and the course Active/Inactive toggle locally and commit them on a **Save changes** button (disabled until dirty; commits `active → removes → adds → access` via `mutateAsync`; unsaved edits discarded on navigate). The per-remove confirm was dropped (staged removal is reversible); Delete course/instructor + Duplicate stay immediate. Tests rewritten to stage-then-Save. Build ✓, lint 0 errors, 280 tests ✓. |
| 2026-07-20 | **Data-router migration + app-wide unsaved-changes guard (`useBlocker`), user-requested — supersedes the context guard (engineering-log ADR-012→013).** `AppV2` moved from the declarative `<BrowserRouter>` to a **data router** (`createBrowserRouter(routes)` + `<RouterProvider>`); `AppRoutes` now exports a `routes` tree (`createRoutesFromElements`) under a new **`RootLayout`** route element that holds `CourseProvider` + the lazy-route `Suspense` (both read the location, so they must live inside the router). `OfflineBanner` + `ToastContainer` stay in `AppV2` outside the router. The `UnsavedChangesContext` was deleted and replaced by a reusable **`<UnsavedChangesPrompt when={isDirty} onProceed?={fn} />`** (`useBlocker` + `beforeunload` + the same "Unsaved changes / Leave without saving" confirm) — now catching **browser back/forward + all in-app nav**, not just the admin list/tab clicks. Wired into `InstructorDetail`/`CourseDetail` (the `runGuarded` wiring in `InstructorList`/`CourseList`/`AdminLayout` was reverted to direct navigation) **and the four forms** (`SettingsTab`, `CreateCourse`, `EditModule`, `CourseWizard`), each with its own dirty signal. Modal save/create navigations bypass via a `leaving`/`leaveTo` flag + effect (no double-prompt); delete bypasses (record gone) while duplicate stays guarded; `CourseWizard` releases its reserved draft via `onProceed={cleanup}`. The guard's behaviour is tested with a real `createMemoryRouter` harness; `AppRoutes.test` drives the same `routes`. Build ✓, lint 0 errors, 284 tests ✓ (95 files). |
