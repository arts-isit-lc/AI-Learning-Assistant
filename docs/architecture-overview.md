...
| **ApiGatewayStack** | API layer + original Lambdas | REST API (WAF), AppSync, student/instructor/admin Lambdas, text_generation, data_ingestion, SQS trigger, S3 upload bucket, Cognito |
...
| **AWS Lambda** | All compute (13 zip + 4 Docker container functions) |
...
| **Claude 3 Haiku** (`anthropic.claude-3-haiku-20240307-v1:0`) | Evaluation, query analysis, topic extraction, vision | V2 chatbotV2Function (Evaluation_Engine), ragRetrievalFunction (QueryAnalyzer), ragEnrichmentFunction (image descriptions), data_ingestion (topic extraction) |
| **Llama 3 70B** (`meta.llama3-70b-instruct-v1:0`) | Alternative response generation | V1 text_generation (instructor-selectable) |
| **Titan Embed v2** (`amazon.titan-embed-text-v2:0`) | Text embeddings (1024 dimensions) | data_ingestion, ragEnrichmentFunction, ragRetrievalFunction |
...
| `Course_Files` | File metadata, processing status, content hashes |
| `langchain_pg_embedding` | Vector embeddings for RAG retrieval (pgvector) |
| `Students`, `Instructors` | User enrollment records |
...
- [System Flow Overview](./system-flow-overview.md) — simplified V1 explanation (still valid for the original path)
