...
| `ErrorState` | `title?`, `description?`, `onRetry?`, `retryLabel?`, `icon?`, `className?` | Inline failed-read placeholder (`role="alert"` + a Retry `Button` when `onRetry` is set). Primary read → full-region; secondary read → panel-scoped (`className="border-0"`). Sibling to `EmptyState`; copy via `toUserMessage(error)` |
