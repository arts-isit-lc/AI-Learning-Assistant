...
- **Server state → TanStack Query.** All REST data (courses, modules, prompts, analytics, roster, sessions). No raw `fetch()` + `useEffect` fetching in components.
  - **Data layer (landed Phase 3):** hooks in `src/services/queries/` (import from the barrel `@/services/queries`), Zod contracts in `src/services/schemas/`, keys via `src/services/queryKeys.js`. Every call goes through `src/services/http.js` — `withAuth` (401 → silent refresh → retry once → clear auth; 403 → inline) + `parseWith` (validates the response against its Zod contract; strict in dev). `createQueryClient` (`queryClient.js`) is provided **only inside `AppV2`**. Retries: GET ×2 backoff, **mutations never**. Errors are logged by a global seam (no toasts — the app has no transient notifications) and surfaced **inline** by screens (`ErrorState` for reads, `Alert`/`FormField` for writes, `ConfirmDialog`'s `error` prop for deletes); the global seam skips `error.inline` (403) and `meta.suppressGlobalError`. **Convention:** any mutation that renders its own inline error (a form-top `Alert`, an in-dialog `ConfirmDialog` error, or an optimistic-revert signal) MUST set `meta: { suppressGlobalError: true }` so the same failure isn't double-recorded by the seam. Instructor/admin hooks are added in their feature phases (P6/P7).
...
- **Form state → React Hook Form + Zod.** Field/validation errors render inline via the form — never globally.
