...
- Chatbot response latency (~10s/turn, noted 2026-07-01): `chatbot_v2` streams the answer with **Claude 3 Sonnet** (`constants/models.py` `RESPONSE_MODEL_ID`, `max_tokens=4000`) and evaluates with **Haiku**. Retrieval-side optimization flags are already ON in the stack — `RAG_RETURN_PASSAGES` (eliminates the double generation), `STRICT_IMAGE_ESCALATION`, `QUERY_EMBEDDING_CACHE` — plus `PARALLEL_EVAL_RETRIEVAL` + `CACHE_MODULE_METADATA`. Biggest remaining lever: upgrade the response model to Claude 3.5 Sonnet/Haiku (faster + ≥ quality). `ragRetrievalFunction` is a Docker Lambda in VPC (1024 MB) → cold starts add seconds. Handlers log `bedrock_call` `latency_ms` — measure via CloudWatch Logs Insights before tuning. [Active]
