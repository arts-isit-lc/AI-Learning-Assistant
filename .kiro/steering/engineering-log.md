...
- Chatbot response latency (~10s/turn, noted 2026-07-01): `chatbot_v2` streams the answer with **Claude 3 Sonnet** (`constants/models.py` `RESPONSE_MODEL_ID`, `max_tokens=4000`) and evaluates with **Haiku**. Retrieval-side optimization flags are already ON in the stack — `RAG_RETURN_PASSAGES` (eliminates the double generation), `STRICT_IMAGE_ESCALATION`, `QUERY_EMBEDDING_CACHE` — plus `PARALLEL_EVAL_RETRIEVAL` + `CACHE_MODULE_METADATA`. **Model swap is NOT an option: `ca-central-1` grants this account access only to Claude 3 Sonnet + Claude 3 Haiku (confirmed 2026-07-01; the newer 4.x/5 models are listed in-region but not access-enabled, and 3.5 Sonnet is not offered at all).** So model-preserving levers only: lower `RESPONSE_MAX_TOKENS` + prompt for concision (cuts generation time directly), trim RAG-context size injected into the prompt (cuts TTFT/prefill), and reduce `ragRetrievalFunction` cold starts (Docker Lambda in VPC, 1024 MB — provisioned concurrency / memory bump). Measure first: handlers log `bedrock_call` `latency_ms` — use CloudWatch Logs Insights to see whether time-to-first-token (pre-generation) or generation length dominates before tuning. [Active]
