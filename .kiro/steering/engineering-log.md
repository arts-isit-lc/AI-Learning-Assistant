...
- **Region model access: `ca-central-1` grants Claude 3 Sonnet + Claude 3 Haiku only. Claude 3.5 (any variant) is NOT available here — never recommend or switch to it.**
- Prompt-conflict validation model: **Claude 3 Sonnet** (`anthropic.claude-3-sonnet-20240229-v1:0`), runtime-configurable via the `/AILA/${env}/ValidationModelId` SSM parameter; `dbLambdaRole` may invoke Sonnet + Haiku. Switched from Haiku for precision — see ADR-005.
