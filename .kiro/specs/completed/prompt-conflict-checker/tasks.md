...
- [x] 3. CDK — Add Claude 3 Haiku to Bedrock Permissions and SSM Parameter: In `api-gateway-stack.ts`, add `anthropic.claude-3-haiku-20240307-v1:0` to existing Bedrock InvokeModel resources array. Create SSM StringParameter `/AILA/${environment}/ValidationModelId`. Add `VALIDATION_MODEL_ID` env var to instructorFunction. Add scoped SSM read permission. Add `POST /instructor/validate_prompt` route to OpenAPI definition. Run `npx tsc --noEmit`. (Req 8, Req 9)
