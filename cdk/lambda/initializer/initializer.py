...
def handler(event, context):
    ...
    try:
        ...
        sqlTableCreation = """
            ...
            -- Access-code uniqueness: course_access_code is the join key students
            -- use to enrol (the student handler resolves a course by
            -- course_access_code with LIMIT 1), so a code must map to exactly one
            -- course — a collision would silently route a student to whichever row
            -- Postgres returned first. Codes are randomly generated by the
            -- create/duplicate/regenerate handlers (collisions are astronomically
            -- unlikely), but this unique index is the guarantee: those handlers
            -- catch the 23505 on this index and regenerate. Applied across ALL
            -- courses (not just active) so a code can never be reused. The column
            -- is nullable and NULLs are DISTINCT under a plain unique index, so
            -- any legacy code-less rows don't collide with each other.
            CREATE UNIQUE INDEX IF NOT EXISTS ux_courses_access_code
            ON "Courses" (course_access_code);
        """
