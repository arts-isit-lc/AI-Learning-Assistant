...
/**
 * Full-width Learning Journey bar (Figma course + module-chat frames): label +
 * overall status + `NN% (x/y concepts completed)`, with a book/chevron trigger
 * that opens the per-concept progress drawer. Shared by CourseView + StudentChat.
 *
 * @param {{ concepts: Array, completedConcepts: number, totalConcepts: number, percent: number }} props
 */
export function LearningJourneyBar({ concepts = [], completedConcepts = 0, totalConcepts = 0, percent = 0 }) {
  const status =
    percent === 100
