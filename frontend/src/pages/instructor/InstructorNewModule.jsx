...
export const InstructorNewModule = ({ courseId }) => {
  ...
  // --- Auto-generation idempotence guard ---
  ...
  // --- Auto-generate topics when all tracked files reach terminal state ---
  useEffect(() => {
    if (!shouldAutoGenerate(trackedFiles)) return;
    if (hasAutoGeneratedRef.current) return;
    if (isGeneratingTopics) return;
    hasAutoGeneratedRef.current = true;
    handleAutoGenerateTopics();
  }, [trackedFiles, isGeneratingTopics]);
  ...
  const handleAutoGenerateTopics = async () => {
    ...
    try {
      ...
      if (result.status === "processing") {
        ...
        const merged = mergeTopics(keyTopics, result.topics);
        setKeyTopics(merged);
  ...
  return (
    <PageContainer>
      <Paper style={{ padding: 25, width: "100%", overflow: "auto" }}>
        ...
        {/* Topic generation loading indicator */}
        {isGeneratingTopics && (
