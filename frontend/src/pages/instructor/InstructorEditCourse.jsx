...
const InstructorEditCourse = () => {
  ...
  // Auto-generate topics when all tracked files finish processing
  const hasAutoGeneratedRef = useRef(false);
  useEffect(() => {
    const entries = Object.values(trackedFiles);
    if (entries.length === 0) return;
    const allComplete = entries.every((f) => f.status === "complete");
    if (allComplete && !hasAutoGeneratedRef.current && !isGeneratingTopics) {
    ...
    <PageContainer>
      <Paper style={{ padding: 25, width: "100%", overflow: "auto" }}>
        ...
        <Box sx={{ marginTop: 3, marginBottom: 1 }}>
          ...
          {/* Populate from generated topics button */}
          {moduleTopics && moduleTopics.topics && moduleTopics.topics.length > 0 && keyTopics.length === 0 && (
            <Button
              size="small"
              variant="outlined"
              sx={{ mb: 1 }}
              onClick={() => setKeyTopics([...moduleTopics.topics])}
            >
              Populate from generated topics
            </Button>
          )}
