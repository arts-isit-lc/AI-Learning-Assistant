  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // the selected row drops it for a solid rounded brand fill.
        "group flex w-full items-center gap-3 border-b border-border px-3 py-3 text-left transition-colors duration-fast",
        interactive &&
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring",
        interactive && !selected && "hover:bg-accent",
        // Selected = solid brand fill. Children with explicit text colours invert
        // via `group-aria-[current=true]:` (in the feature rows); plain-text
        // children inherit `text-primary-foreground` here.
        selected && "rounded-md border-transparent bg-primary text-primary-foreground",
        className
      )}
    >
      {children}
